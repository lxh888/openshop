...
use eapie\main;
use eapie\error;
class admin_credit extends \eapie\source\request\user {
	 ...
	 * 根据检索条件获取用户积分总数
	 * 
	 * USERADMINCREDITTOTAL
	 * {"class":"user/admin_credit","method":"api_total"}
	 * 
	 * @param	array		$search
	 * @return	int
	 */
	public function api_total($search = array()){
		...
		if(!empty($search)){
			if( isset($search['user_id']) && is_string($search['user_id']) ){
				$where[] = array('[and] uc.user_id=[+]', $search['user_id']);
			}
			...
			if (isset($search['user_nickname']) && is_string($search['user_nickname'])) {
				...
				$where[] = array('[and] uc.user_id=[+]', $user_id);
            }
			...
			(int)$search['min_value'] >= 0 ) {
				$where[] = array('uc.user_credit_value > [-]', ( (int)$search['min_value'] - 1) );
            }
			
			if (isset($search['max_value']) && 
			is_numeric($search['max_value']) && 
			(int)$search['max_value'] >= 0 ) {
				$where[] = array('uc.user_credit_value < [-]', ( (int)$search['max_value'] + 1) );
            }
			
		}
		
		return object(parent::TABLE_USER_CREDIT)->find_now_where_sum($where);
	}
	 ...
	 * 
	 * USERADMINCREDITLIST
	 * {"class":"user/admin_credit","method":"api_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_list($data = array()){
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'value_desc' => array('user_credit_value', true),
			'value_asc' =>  array('user_credit_value', false),
			
			'time_desc' => array('user_credit_time', true),
			'time_asc' => array('user_credit_time', false),
		));
		...
		if(!empty($data['search'])){
			...
			(int)$data['search']['min_value'] >= 0 ) {
				$sql_join_now_value = object(parent::TABLE_USER_CREDIT)->sql_join_user_now_value("u");
		...
		return object(parent::TABLE_USER_CREDIT)->select_user_page($config);
	 ...
	 * 获取流水号列表
	 * 
	 * USERADMINCREDITSERIALLIST
	 * {"class":"user/admin_credit","method":"api_serial_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_serial_list($data = array()){
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'type_desc' => array('user_credit_type', true),
			'type_asc' => array('user_credit_type', false),
		...
		//避免排序重复
		$config["orderby"][] = array('user_credit_id', false);
		...
		if(!empty($data['search'])){
			...
			if( isset($data['search']['user_credit_id']) && is_string($data['search']['user_credit_id']) ){
				$config["where"][] = array('[and] uc.user_credit_id=[+]', $data['search']['user_credit_id']);
			}
			if( isset($data['search']['type']) && is_string($data['search']['type']) ){
				$config["where"][] = array('[and] uc.user_credit_type=[+]', $data['search']['type']);
			}
			if( isset($data['search']['type_name']) && is_string($data['search']['type_name']) && !empty($type_list) ){
				foreach($type_list as $type_k => $type_v){
					if(mb_strstr($type_v, $data['search']['type_name']) !== false){
						$config["where"][] = array('[and] uc.user_credit_type=[+]', $type_k);
						break;
					}
				}
			}
		...
		$data_list = object(parent::TABLE_USER_CREDIT)->select_serial_page($config);
		
		if( !empty($data_list["data"]) && !empty($type_list) ){
			foreach($data_list["data"] as $key => $value){
				if( isset($type_list[$value["user_credit_type"]]) ){
					$data_list["data"][$key]["user_credit_type_name"] = $type_list[$value["user_credit_type"]];
				}
			}
		}
	 ...
	 * 添加/减少积分
	 * USERADMINCREDITEDIT
	 * {"class":"user/admin_credit","method":"api_edit"}
	 * 
	 * @param	array	$data
	 * @return	bool
	 */
	public function api_edit($data = array()){
		...
		if($data['type'] == parent::TRANSACTION_TYPE_ADMIN_MINUS){
			//如果是人工减少
			//获取积分  判断积分
			$user_credit_data = object(parent::TABLE_USER_CREDIT)->find_now_data($data['user_id']);
			if( empty($user_credit_data["user_credit_value"]) 
			|| ($user_credit_data["user_credit_value"] - $data['value']) < 0 ){
				throw new error('该用户积分不足');
			}
		...
		$bool = object(parent::TABLE_USER_CREDIT)->insert_admin(array(
			"admin_user_id" => $_SESSION["user_id"],
			"user_id" => $data['user_id'],
			"comment" => $data['comment'],
			"value" => $data['value'],
			"type" => $data['type'],
			"user_credit" => $user_credit_data,
		));
