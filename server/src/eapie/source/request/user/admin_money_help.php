...
class admin_money_help extends \eapie\source\request\user
{
	 ...
	 * 根据检索条件获取用户扶贫基金资金总数
	 * 
	 * USERADMINMONEYHELPTOTAL
	 * {"class":"user/admin_money_help","method":"api_total"}
	 * 
	 * @param	array		$search
	 * @return	int
	 */
	...
	{
		//检查权限
		object(parent::REQUEST_ADMIN)->check(parent::AUTHORITY_MONEY_HELP_READ);
		...
		if (!empty($search)) {
			...
			if (
				isset($search['min_value']) &&
				is_numeric($search['min_value']) &&
				(int)$search['min_value'] >= 0
			) {
				$where[] = array('ume.user_money_help_value > [-]', ((int)$search['min_value'] - 1));
			}

			if (
				isset($search['max_value']) &&
				is_numeric($search['max_value']) &&
				(int)$search['max_value'] >= 0
			) {
				$where[] = array('ume.user_money_help_value < [-]', ((int)$search['max_value'] + 1));
			}
		}

		return object(parent::TABLE_USER_MONEY_HELP)->find_now_where_sum($where);
	}
	 ...
	 * );
	 * 
	 * USERADMINMONEYHELPLIST
	 * {"class":"user/admin_money_help","method":"api_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_list($data = array())
	{
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'value_desc' => array('user_money_help_value', true),
			'value_asc' =>  array('user_money_help_value', false),

			'time_desc' => array('user_money_help_time', true),
			'time_asc' => array('user_money_help_time', false),
		));
		...
		if (!empty($data['search'])) {
			...
			if (
				isset($data['search']['min_value']) &&
				is_numeric($data['search']['min_value']) &&
				(int)$data['search']['min_value'] >= 0
			) {
				$sql_join_now_value = object(parent::TABLE_USER_MONEY_HELP)->sql_join_user_now_value("u");
				$config['where'][] = array('[and] (' . $sql_join_now_value . ') > []', ((int)$data['search']['min_value'] - 1));
			}

			if (
				isset($data['search']['max_value']) &&
		...
		return object(parent::TABLE_USER_MONEY_HELP)->select_user_page($config);
	}
	 ...
	 * 获取流水号列表
	 * 
	 * USERADMINMONEYHELPSERIALLIST
	 * {"class":"user/admin_money_help","method":"api_serial_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_serial_list($data = array())
	{
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'type_desc' => array('user_money_help_type', true),
			'type_asc' => array('user_money_help_type', false),
		...
		//避免排序重复
		$config["orderby"][] = array('user_money_help_id', false);
		...
		if (!empty($data['search'])) {
			...
			if (isset($data['search']['user_money_help_id']) && is_string($data['search']['user_money_help_id'])) {
				$config["where"][] = array('[and] ume.user_money_help_id=[+]', $data['search']['user_money_help_id']);
			}
			if (isset($data['search']['type']) && is_string($data['search']['type'])) {
				$config["where"][] = array('[and] ume.user_money_help_type=[+]', $data['search']['type']);
			}
			if (isset($data['search']['type_name']) && is_string($data['search']['type_name']) && !empty($type_list)) {
				foreach ($type_list as $type_k => $type_v) {
					if (mb_strstr($type_v, $data['search']['type_name']) !== false) {
						$config["where"][] = array('[and] ume.user_money_help_type=[+]', $type_k);
						break;
					}
				}
			}
		}
		$data_list = object(parent::TABLE_USER_MONEY_HELP)->select_serial_page($config);

		if (!empty($data_list["data"]) && !empty($type_list)) {
			foreach ($data_list["data"] as $key => $value) {
				if (isset($type_list[$value["user_money_help_type"]])) {
					$data_list["data"][$key]["user_money_help_type_name"] = $type_list[$value["user_money_help_type"]];
				}
			}
		}
	 ...
	 * 添加/减少用户扶贫基金
	 * 
	 * USERADMINMONEYHELPEDIT
	 * {"class":"user/admin_money_help","method":"api_edit"}
	 * 
	 * @param	array	$data
	 * @return	bool
	 */
	...
	{
		//检查权限
		object(parent::REQUEST_ADMIN)->check(parent::AUTHORITY_MONEY_HELP_EDIT);
		...
		if ($data['type'] == parent::TRANSACTION_TYPE_ADMIN_MINUS) {
			//如果是人工减少
			//获取商户的库存积分  判断要赠送的积分
			$user_money_help_data = object(parent::TABLE_USER_MONEY_HELP)->find_now_data($data['user_id']);

			if (
				empty($user_money_help_data["user_money_help_value"])
				|| ($user_money_help_data["user_money_help_value"] - $data['value']) < 0
			) {
				throw new error('该用户扶贫基金的余额不足');
			}
			
		} else
		if ($data['type'] == parent::TRANSACTION_TYPE_ADMIN_PLUS) {
			$user_money_help_data = array();
		} else {
			throw new error('交易类型异常');
		}

		$bool = object(parent::TABLE_USER_MONEY_HELP)->insert_admin(array(
			"admin_user_id" => $_SESSION["user_id"],
			"user_id" => $data['user_id'],
			"comment" => $data['comment'],
			"value" => $data['value'],
			"type" => $data['type'],
			"user_money_help" => $user_money_help_data,
		));
	 ...
	 * }
	 * 
	 * {"class":"user/admin_money_help","method":"api_excel"}
	 * 
	 * @param	array
	 * @return	file
	 */
	public function api_excel($data = array())
	{
		...
		// $data = array(
		// 	// 'phone' => '12345678977',
		// 	'start_time' => cmd(array('2018-06-04 04:04:03'), "time mktime"),
		// 	'end_time' => cmd(array('2019-07-26 04:04:03'), "time mktime"),
		// 	// 期末余额为零
		// 	'balance_is_zero' => true,
		// 	// 当期发生额为零
		// 	'occurrence_is_zero' => true
		// );

		// 检查权限
		object(parent::REQUEST_ADMIN)->check(parent::AUTHORITY_MONEY_HELP_EXCEL);

		// $data['start_time'] = "2019-06-10 04:04:03";
		...
		$money_help_config = array(
			'orderby' => array(
				array('user_money_help_time', true),
				array('user_id'),
			),
			'where' => array(),
			// 'limit' => array(0, 1000),
		...
		if (isset($data['phone']) && is_string($data['phone'])) {
			...
			$money_help_config['where'][] = array('user_id=[+]', $user_data['user_id']);
		}

		// 根据时间段进行筛选

		// if (isset($data['start_time']) && isset($data['end_time'])) {
		// 	$money_help_config['where'][] = array("[and] user_money_help_time between []", $start_time);
		// 	$money_help_config['where'][] = array("[and] []", $end_time);
		// }
		$money_help_config['where'][] = array("[and] user_money_help_time between []", $start_time);
		$money_help_config['where'][] = array("[and] []", $end_time);

		// 查询数据
		$user_money_help = object(parent::TABLE_USER_MONEY_HELP)->select($money_help_config);
		if (empty($user_money_help)) {
			throw new error('当前条件下数据为空');
		}

		// 所有用户ID
		$user_ids = array();
		foreach ($user_money_help as $v) {
			...
			foreach ($user_datas_group as $k => $val) {
				if ($k == $u['user_id']) {
					foreach ($val as $k1 => $v1) {
						if ($k1 == 0) {
							// 期末余额
							$u['balance'] = $v1['user_money_help_value'];
						}
						if ($v1['user_money_help_plus'] != 0) {
							$u['occurrence'] += $v1['user_money_help_plus'];
						} else {
							$u['occurrence'] -= $v1['user_money_help_minus'];
						}
					}
				}
			}
