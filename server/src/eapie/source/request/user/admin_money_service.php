...
use eapie\main;
use eapie\error;
class admin_money_service extends \eapie\source\request\user {
	 ...
	 * 根据检索条件获取用户服务费资金总数
	 * 
	 * USERADMINMONEYSERVICETOTAL
	 * {"class":"user/admin_money_service","method":"api_total"}
	 * 
	 * @param	array		$search
	 * @return	int
	 */
	public function api_total($search = array()){
		//检查权限
		object(parent::REQUEST_ADMIN)->check(parent::AUTHORITY_MONEY_SERVICE_READ);
		...
		if(!empty($search)){
			...
			(int)$search['min_value'] >= 0 ) {
				$where[] = array('ums.user_money_service_value > [-]', ( (int)$search['min_value'] - 1) );
            }
			
			if (isset($search['max_value']) && 
			is_numeric($search['max_value']) && 
			(int)$search['max_value'] >= 0 ) {
				$where[] = array('ums.user_money_service_value < [-]', ( (int)$search['max_value'] + 1) );
            }
			
		}
		
		return object(parent::TABLE_USER_MONEY_SERVICE)->find_now_where_sum($where);
	}
	 ...
	 * );
	 * 
	 * USERADMINMONEYSERVICELIST
	 * {"class":"user/admin_money_service","method":"api_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_list($data = array()){
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'value_desc' => array('user_money_service_value', true),
			'value_asc' =>  array('user_money_service_value', false),
			
			'time_desc' => array('user_money_service_time', true),
			'time_asc' => array('user_money_service_time', false),
		));
		...
		if(!empty($data['search'])){
			...
			(int)$data['search']['min_value'] >= 0 ) {
				$sql_join_now_value = object(parent::TABLE_USER_MONEY_SERVICE)->sql_join_user_now_value("u");
		...
		return object(parent::TABLE_USER_MONEY_SERVICE)->select_user_page($config);
	 ...
	 * 获取流水号列表
	 * 
	 * USERADMINMONEYSERVICESERIALLIST
	 * {"class":"user/admin_money_service","method":"api_serial_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_serial_list($data = array()){
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'type_desc' => array('user_money_service_type', true),
			'type_asc' => array('user_money_service_type', false),
		...
		//避免排序重复
		$config["orderby"][] = array('user_money_service_id', false);
		...
		if(!empty($data['search'])){
			...
			if (isset($data['search']['order_action_user_id']) && is_string($data['search']['order_action_user_id'])) {
                $config['where'][] = array('[and] oau.user_id=[+]', $data['search']['order_action_user_id']);
            }
			if (isset($data['search']['order_action_user_nickname']) && is_string($data['search']['order_action_user_nickname'])) {
                $config['where'][] = array('[and] oau.user_nickname LIKE "%[-]%"', $data['search']['order_action_user_nickname']);
            }
			if (isset($data['search']['order_action_user_phone']) && is_string($data['search']['order_action_user_phone'])) {
                ...
                $config['where'][] = array('[and] oau.user_id=[+]', $user_id);
            }
			
			if( isset($data['search']['user_money_service_id']) && is_string($data['search']['user_money_service_id']) ){
				$config["where"][] = array('[and] ums.user_money_service_id=[+]', $data['search']['user_money_service_id']);
			}
			if( isset($data['search']['type']) && is_string($data['search']['type']) ){
				$config["where"][] = array('[and] ums.user_money_service_type=[+]', $data['search']['type']);
			}
			if( isset($data['search']['type_name']) && is_string($data['search']['type_name']) && !empty($type_list) ){
				foreach($type_list as $type_k => $type_v){
					if(mb_strstr($type_v, $data['search']['type_name']) !== false){
						$config["where"][] = array('[and] ums.user_money_service_type=[+]', $type_k);
						break;
					}
				}
			}
		...
		$data_list = object(parent::TABLE_USER_MONEY_SERVICE)->select_serial_page($config);
		
		if( !empty($data_list["data"]) && !empty($type_list) ){
			foreach($data_list["data"] as $key => $value){
				if( isset($type_list[$value["user_money_service_type"]]) ){
					$data_list["data"][$key]["user_money_service_type_name"] = $type_list[$value["user_money_service_type"]];
				}
			}
		}
	 ...
	 * }
	 * 
	 * {"class":"user/admin_money_service","method":"api_excel"}
	 * 
	 * @param	array
	 * @return	file
	 */
	public function api_excel($data = array())
	{
		...
		// 检查权限
		object(parent::REQUEST_ADMIN)->check(parent::AUTHORITY_MONEY_SERVICE_EXCEL);
		...
		$money_service_config = array(
			'orderby' => array(
				array('user_money_service_time', true),
				array('user_id'),
			),
			'where' => array(),
			// 'limit' => array(0, 1000),
		...
		if (isset($data['phone']) && is_string($data['phone'])) {
			...
			$money_service_config['where'][] = array('user_id=[+]', $user_data['user_id']);
		}

		// 根据时间段进行筛选

		// if (isset($data['start_time']) && isset($data['end_time'])) {
		// 	$money_service_config['where'][] = array("[and] user_money_service_time between []", $start_time);
		// 	$money_service_config['where'][] = array("[and] []", $end_time);
		// }
		$money_service_config['where'][] = array("[and] user_money_service_time between []", $start_time);
		$money_service_config['where'][] = array("[and] []", $end_time);

		// 查询数据
		$user_money_service = object(parent::TABLE_USER_MONEY_SERVICE)->select($money_service_config);
		if (empty($user_money_service)) {
			throw new error('当前条件下数据为空');
		}

		// 所有用户ID
		$user_ids = array();
		foreach ($user_money_service as $v) {
			...
			foreach ($user_datas_group as $k => $val) {
				if ($k == $u['user_id']) {
					foreach ($val as $k1 => $v1) {
						if ($k1 == 0) {
							// 期末余额
							$u['balance'] = $v1['user_money_service_value'];
						}
						if ($v1['user_money_service_plus'] != 0) {
							$u['occurrence'] += $v1['user_money_service_plus'];
						} else {
							$u['occurrence'] -= $v1['user_money_service_minus'];
						}
					}
				}
			}
