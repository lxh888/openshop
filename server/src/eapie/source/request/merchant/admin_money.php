...
use eapie\main;
use eapie\error;
class admin_money extends \eapie\source\request\merchant {
	 ...
	 * 根据检索条件获取商家钱包总余额
	 * 
	 * MERCHANTADMINMONEYTOTAL
	 * {"class":"merchant/admin_money","method":"api_total"}
	 * 
	 * @param	array		$search
	 * @return	int
	 */
	public function api_total($search = array()){
		...
		if(!empty($search)){
			if( isset($search['merchant_id']) && is_string($search['merchant_id']) ){
				$config["where"][] = array('[and] mm.merchant_id=[+]', $search['merchant_id']);
			}
			if (isset($search['merchant_name']) && is_string($search['merchant_name'])) {
				...
				$where[] = array('[and] mm.merchant_id=[+]', $merchant_id);
            }
			
			if (isset($search['min_value']) && 
			is_numeric($search['min_value']) && 
			(int)$search['min_value'] >= 0 ) {
				$where[] = array('mm.merchant_money_value > [-]', ( (int)$search['min_value'] - 1) );
            }
			
			if (isset($search['max_value']) && 
			is_numeric($search['max_value']) && 
			(int)$search['max_value'] >= 0 ) {
				$where[] = array('mm.merchant_money_value < [-]', ( (int)$search['max_value'] + 1) );
            }
			
		}
		
		return object(parent::TABLE_MERCHANT_MONEY)->find_now_where_sum($where);
	}
	 ...
	 * );
	 * 
	 * MERCHANTADMINMONEYLIST
	 * {"class":"merchant/admin_money","method":"api_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_list($data = array()){
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'value_desc' => array('merchant_money_value', true),
			'value_asc' =>  array('merchant_money_value', false),
			
			'time_desc' => array('merchant_money_time', true),
			'time_asc' => array('merchant_money_time', false),
		));
		...
		if(!empty($data['search'])){
			...
			(int)$data['search']['min_value'] >= 0 ) {
				$sql_join_now_value = object(parent::TABLE_MERCHANT_MONEY)->sql_join_merchant_now_value("m");
		...
		return object(parent::TABLE_MERCHANT_MONEY)->select_merchant_page($config);
	 ...
	 * 获取流水号列表
	 * 
	 * MERCHANTADMINMONEYSERIALLIST
	 * {"class":"merchant/admin_money","method":"api_serial_list"}
	 * 
	 * @param	array	$data
	 * @return	array
	 */
	public function api_serial_list($data = array()){
		...
		$config["orderby"] = object(parent::REQUEST)->orderby($data, array(
			...
			'type_desc' => array('merchant_money_type', true),
			'type_asc' => array('merchant_money_type', false),
		...
		//避免排序重复
		$config["orderby"][] = array('merchant_money_id', false);
		$type_list = object(parent::TABLE_ORDER)->get_type();
		
		
		//时间范围
		if( isset($data['time_horizon']) && 
		!empty($data['time_horizon']['start_time']) &&
		!empty($data['time_horizon']['end_time']) ){
			$start_time = cmd(array($data['time_horizon']['start_time']), "time mktime");
			$end_time = cmd(array($data['time_horizon']['end_time']), "time mktime");
			if( is_numeric($start_time) && is_numeric($end_time) ){
				$config["where"][] = array('[and] mm.merchant_money_time>=[-]', $start_time);
				$config["where"][] = array('[and] mm.merchant_money_time<=[-]', $end_time);
			}
		}
		...
		if(!empty($data['search'])){
			...
			if( isset($data['search']['merchant_money_id']) && is_string($data['search']['merchant_money_id']) ){
				$config["where"][] = array('[and] mm.merchant_money_id=[+]', $data['search']['merchant_money_id']);
			}
			if( isset($data['search']['type']) && is_string($data['search']['type']) ){
				$config["where"][] = array('[and] mm.merchant_money_type=[+]', $data['search']['type']);
			}
			if( isset($data['search']['type_name']) && is_string($data['search']['type_name']) && !empty($type_list) ){
				foreach($type_list as $type_k => $type_v){
					if(mb_strstr($type_v, $data['search']['type_name']) !== false){
						$config["where"][] = array('[and] mm.merchant_money_type=[+]', $type_k);
						break;
					}
				}
			}
		...
		$data_list = object(parent::TABLE_MERCHANT_MONEY)->select_serial_page($config);
		
		if( !empty($data_list["data"]) && !empty($type_list) ){
			foreach($data_list["data"] as $key => $value){
				if( isset($type_list[$value["merchant_money_type"]]) ){
					$data_list["data"][$key]["merchant_money_type_name"] = $type_list[$value["merchant_money_type"]];
				}
			}
		}
	...
	/**
	 * 添加/减少余额
	 * MERCHANTADMINMONEYEDIT
	 * {"class":"merchant/admin_money","method":"api_edit"}
	 * 
	 * @param	array	$data
	 * @return	bool
	 */
	public function api_edit($data = array()){
		...
		if($data['type'] == parent::TRANSACTION_TYPE_ADMIN_MINUS){
			//如果是人工减少
			//获取商户的库存积分  判断要赠送的积分
			$merchant_money_data = object(parent::TABLE_MERCHANT_MONEY)->find_now_data($data['merchant_id']);
			if( empty($merchant_money_data["merchant_money_value"]) 
			|| ($merchant_money_data["merchant_money_value"] - $data['value']) < 0 ){
				throw new error('该商户钱包余额不足');
			}
			
		}else
		if($data['type'] == parent::TRANSACTION_TYPE_ADMIN_PLUS){
			$merchant_money_data = array();
		}else{
			throw new error('交易类型异常');
		}
		
		$bool = object(parent::TABLE_MERCHANT_MONEY)->insert_admin(array(
			"admin_user_id" => $_SESSION["user_id"],
			"merchant_id" => $data['merchant_id'],
			"comment" => $data['comment'],
			"value" => $data['value'],
			"type" => $data['type'],
			"merchant_money" => $merchant_money_data,
		));
