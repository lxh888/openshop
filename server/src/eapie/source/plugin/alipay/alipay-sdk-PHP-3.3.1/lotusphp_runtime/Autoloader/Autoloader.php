...
class LtAutoloader
{
	...
	protected function addFileMap($file)
	{
		...
		{
			//wangaho update Error:: filemtime(): stat failed for
			$fileStoreGetFilePath = $this->fileStore->getFilePath($file);
			if( !is_file($fileStoreGetFilePath) ){
				$cachedFileLastModified = 0;
			}else{
				$cachedFileLastModified = (int) @filemtime($fileStoreGetFilePath);
			}
