WangAho({
	...
	main : function(){
		...
		WangAho("index").data({
			request : {
				list:["HOUSEADMINPRODUCTTRASHLIST", [config]]
			},
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/house/product_trash.html", "#content"), data);
				_project.event();
	...
	event : function(){
		...
		$('[action-button]').unbind("click").click(function(){
			...
			//还原
			if("restore" == attr){
				layer.msg('你确定要还原么？('+ids.length+'条数据)', {time: 0 //不自动关闭
					,btn: ['确定', '取消']
					,yes: function(index){
					    layer.close(index);
					    _this.restore(ids);
	...
	restore : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["HOUSEADMINPRODUCTTRASHRESTORE", [{house_product_id:ids[i]}]]);
		}
