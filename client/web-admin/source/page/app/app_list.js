WangAho({
	...
	main : function(){
		...
		if( (function(){try{ return _http.anchor.query.state;}catch(e){return false;}}()) ){
			config.search.app_state = _http.anchor.query.state;
		}
		...
		var request = {list:["APPADMINLIST", [config]]};
		...
		WangAho("index").data({
			...
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/app/app_list.html", "#content"), data, {
	...
	search_event : function(){
		...
		$(".search").unbind("click").click(function(){
			...
			layer.open({
				title : "<span class=\"glyphicon glyphicon-filter\"></span> 筛选",
			  	...
			  	content: template( WangAho().template("page/app/app_list.html", "#search"), function(fn){
		...
		$('[name="search-submit"]').unbind("click").click(function(){
			...
			var search = {};
			search.app_id = $.trim($('[name="app_id"]').val());
			search.app_name = $.trim($('[name="app_name"]').val());
			for(var i in search){
				if(search[i] == ""){
					delete search[i];
				}
	...
	edit_yes_self : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["MERCHANTADMINEDITSELF", [{app_id:ids[i],app_self:1}]]);
		}
	...
	edit_not_self : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["MERCHANTADMINEDITSELF", [{app_id:ids[i],app_self:0}]]);
		}
	...
	remove : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["MERCHANTADMINREMOVE", [{app_id:ids[i]}]]);
		}
