WangAho({
	...
	id:"administrator/authority_list",
	...
	main : function(){
		...
		var request = {
			...
			list:["ADMINISTRATORADMINAUTHORITYLIST", [config]]
			};
		...
		WangAho("index").data({
			...
			success : function(data){
				...
				_project.data = data;
				WangAho("index").view(WangAho().template("page/administrator/authority_list.html", "#content"), data);
				_project.event();
	...
	search_event : function(){
		...
		$(".search").unbind("click").click(function(){
			...
			layer.open({
				title : "<span class=\"glyphicon glyphicon-filter\"></span> 筛选",
			  	...
			  	content: template( WangAho().template("page/administrator/authority_list.html", "#search"), function(fn){
		...
		$('[name="search-submit"]').unbind("click").click(function(){
			...
			var search = {};
			search.authority_id = $.trim($('[name="authority_id"]').val());
			search.authority_name = $.trim($('[name="authority_name"]').val());
	...
	sort : function(){
		...
		var request_array = [];
		request_array.push(["ADMINISTRATORADMINAUTHORITYEDITCHECK"]);//第一个是判断是否有编辑权限
		for(var i in obj){
			request_array.push(["ADMINISTRATORADMINAUTHORITYEDIT", [{authority_id:obj[i].id, authority_sort:obj[i].value}]]);
		}
	...
	remove : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["ADMINISTRATORADMINAUTHORITYREMOVE", [{authority_id:ids[i]}]]);
		}
