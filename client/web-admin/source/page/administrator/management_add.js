WangAho({
	...
	id:"administrator/management_add",
	...
	main : function(){
		...
		WangAho("index").data({
			...
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/administrator/management_add.html", "#content"), data);
	...
	event : function(){
		...
		$('[name="submit"]').unbind("click").click(function(){
			...
			WangAho("index").submit({
				method:"submit",
				request:["ADMINISTRATORADMINMANAGEMENTADD", [form_input]],
				error:function(){
					$btn.removeClass('disabled');
				},
				success:function(data){
