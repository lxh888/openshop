WangAho({
	...
	main : function(){
		...
		if( !config.api_id ){
			http("#/administrator/api_list").request();
			return false;
		}
		...
		WangAho("index").data({
			request : {
				...
				get:["ADMINISTRATORADMINAPIEDITGET", [config]]
			}, 
			success : function(data){
				...
				//判断数据是否存在
				if( !(function(){try{ return data.response.get.api_id;}catch(e){return false;}}()) ){
					setTimeout(function(){
						if( WangAho().history_previous_exist() ){
							WangAho().history_previous();//存在上一页则返回上一页
						}else{
							WangAho().history_remove();//删除本页的记录
							http("#/administrator/api_list").request();//返回列表页
						}
				...
				WangAho("index").view(WangAho().template("page/administrator/api_edit.html", "#content"), data);
	...
	submit : function(){
		...
		$('[name="submit"]').unbind("click").click(function(){
			...
			WangAho("index").submit({
				method:"submit",
				request:["ADMINISTRATORADMINAPIEDIT", [form_input]],
				error:function(){
					$btn.removeClass('disabled');
				},
				success:function(data){
					
					if( form_input.primary_key != form_input.api_id){
						_http.anchor.query.id = form_input.api_id;
						http(_http).request();
					}else{
						//刷新页面
