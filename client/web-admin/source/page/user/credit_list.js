WangAho({
	...
	main : function(){
		...
		var request = {
				...
				list:["USERADMINCREDITLIST", [config]]
			};
		var template_html = WangAho().template("page/user/credit_list.html", "#content");
		_project.action = (function(){try{ return _http.anchor.query.action;}catch(e){return false;}}());
		if( _project.action == "serial" ){
			request.list = ["USERADMINCREDITSERIALLIST", [config]];
			template_html = WangAho().template("page/user/credit_list.html", "#serial-content");
		}else{
			request.total = ["USERADMINCREDITTOTAL", [config.search]]
		}
	...
	search_event : function(){
		...
		$(".search").unbind("click").click(function(){
			...
			var search_template = WangAho().template("page/user/credit_list.html", "#search");
			if( _project.action == "serial" ){
				search_template = WangAho().template("page/user/credit_list.html", "#serial-search");
			}
		...
		$('[name="search-submit"]').unbind("click").click(function(){
			...
			if( _project.action == "serial" ){
				search.user_credit_id = $.trim($('[name="user_credit_id"]').val());
	...
	event : function(){
		...
		$(".form_plus").unbind("click").click(function(){
			...
			//页面层
			layer.open({
				title : "<span class=\"glyphicon glyphicon-plus\"></span> 增加用户积分",
			  	type: 1,
			  	shadeClose: true,
			  	area: ['auto'], //宽高
			  	maxWidth: 1000,
			  	content: template( WangAho().template("page/user/credit_list.html", "#plus"), function(fn){
					return fn({user_id : user_id, application_config_credit : _application_config_credit});
					})
			});
		...
		$('[name="plus-submit"]').unbind("click").click(function(){
			...
			WangAho("index").submit({
				method:"submit",
				request:["USERADMINCREDITEDIT", [form_input]],
				error:function(){
					$btn.removeClass('disabled');
				},
				success:function(data){
		...
		$(".form_minus").unbind("click").click(function(){
			...
			//页面层
			layer.open({
				title : "<span class=\"glyphicon glyphicon-minus\"></span> 减少用户积分",
			  	type: 1,
			  	shadeClose: true,
			  	area: ['auto'], //宽高
			  	maxWidth: 1000,
			  	content: template( WangAho().template("page/user/credit_list.html", "#minus"), function(fn){
					return fn({user_id : user_id, value : value, application_config_credit : _application_config_credit});
					})
			});
