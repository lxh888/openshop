WangAho({
	...
	main : function(){
		...
		var request = {
				...
				list:["USERADMINMONEYANNUITYLIST", [config]]
			};
		var template_html = WangAho().template("page/user/money_annuity_list.html", "#content");
		_project.action = (function(){try{ return _http.anchor.query.action;}catch(e){return false;}}());
		if( _project.action == "serial" ){
			request.list = ["USERADMINMONEYANNUITYSERIALLIST", [config]];
			template_html = WangAho().template("page/user/money_annuity_list.html", "#serial-content");
		}else{
			request.total = ["USERADMINMONEYANNUITYTOTAL", [config.search]]
		}
	...
	search_event : function(){
		...
		$(".search").unbind("click").click(function(){
			...
			var search_template = WangAho().template("page/user/money_annuity_list.html", "#search");
			if( _project.action == "serial" ){
				search_template = WangAho().template("page/user/money_annuity_list.html", "#serial-search");
			}
		...
		$('[name="search-submit"]').unbind("click").click(function(){
			...
			if( _project.action == "serial" ){
				search.user_money_annuity_id = $.trim($('[name="user_money_annuity_id"]').val());
	...
	event : function(){
		...
		$(".form_plus").unbind("click").click(function(){
			...
			//页面层
			layer.open({
				title : "<span class=\"glyphicon glyphicon-plus\"></span> 增加用户养老基金",
			  	type: 1,
			  	shadeClose: true,
			  	area: ['auto'], //宽高
			  	maxWidth: 1000,
			  	content: template( WangAho().template("page/user/money_annuity_list.html", "#plus"), function(fn){
		...
		$('[name="plus-submit"]').unbind("click").click(function(){
			...
			WangAho("index").submit({
				method:"submit",
				request:["USERADMINMONEYANNUITYEDIT", [form_input]],
				error:function(){
					$btn.removeClass('disabled');
				},
				success:function(data){
		...
		$(".form_minus").unbind("click").click(function(){
			...
			//页面层
			layer.open({
				title : "<span class=\"glyphicon glyphicon-minus\"></span> 减少用户养老基金",
			  	type: 1,
			  	shadeClose: true,
			  	area: ['auto'], //宽高
			  	maxWidth: 1000,
			  	content: template( WangAho().template("page/user/money_annuity_list.html", "#minus"), function(fn){
