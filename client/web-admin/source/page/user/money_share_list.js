WangAho({
	...
	main : function(){
		...
		var request = {
				...
				list:["USERADMINMONEYSHARELIST", [config]]
			};
		var template_html = WangAho().template("page/user/money_share_list.html", "#content");
		_project.action = (function(){try{ return _http.anchor.query.action;}catch(e){return false;}}());
		if( _project.action == "serial" ){
			request.list = ["USERADMINMONEYSHARESERIALLIST", [config]];
			template_html = WangAho().template("page/user/money_share_list.html", "#serial-content");
		}else{
			request.total = ["USERADMINMONEYSHARETOTAL", [config.search]]
		}
	...
	search_event : function(){
		...
		$(".search").unbind("click").click(function(){
			...
			var search_template = WangAho().template("page/user/money_share_list.html", "#search");
			if( _project.action == "serial" ){
				search_template = WangAho().template("page/user/money_share_list.html", "#serial-search");
			}
