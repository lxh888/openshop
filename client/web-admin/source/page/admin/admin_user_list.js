WangAho({
	...
	main : function(){
		...
		var request = {
				...
				list:["ADMINUSERLIST", [config]]
			};
		if( config.search.admin_id ){
			request.admin_get = ["ADMINGET", [{admin_id : config.search.admin_id}]];
		}
		...
		WangAho("index").data({
			...
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/admin/admin_user_list.html", "#content"), data);
				_project.event();
	...
	search_event : function(){
		...
		$(".search").unbind("click").click(function(){
			...
			layer.open({
				title : "<span class=\"glyphicon glyphicon-filter\"></span> 筛选",
			  	...
			  	content: template( WangAho().template("page/admin/admin_user_list.html", "#search"), function(fn){
		...
		$('[name="search-submit"]').unbind("click").click(function(){
			...
			search.admin_id = $.trim($('[name="admin_id"]').val());
			search.admin_name = $.trim($('[name="admin_name"]').val());
			for(var i in search){
				if(search[i] == ""){
					delete search[i];
				}
