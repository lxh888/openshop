WangAho({
	...
	main : function(){
		...
		//状态
		if( (function(){try{ return _http.anchor.query.shop_goods_state;}catch(e){return false;}}()) ){
			config.search.shop_goods_state = _http.anchor.query.shop_goods_state;
		}
		...
		WangAho("index").data({
			request : {
				list:["SHOPADMINGOODSWHENLIST", [config]]
				}, 
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/shop/goods_when_list.html", "#content"), data, {
					
					"anchor-query-shop_goods_state" : function(s){
						var h = http();
						if(!h.anchor.query) h.anchor.query = {};
						if(typeof s == 'undefined'){
							if(typeof h.anchor.query.shop_goods_state != 'undefined') delete h.anchor.query.shop_goods_state;
						}else{
							h.anchor.query.shop_goods_state = s;
						}
		...
		$(".search").unbind("click").click(function(){
			...
			layer.open({
				title : "<span class=\"glyphicon glyphicon-filter\"></span> 筛选",
			  	...
			  	content: template( WangAho().template("page/shop/goods_when_list.html", "#search"), function(fn){
	...
	sort : function(){
		...
		for(var i in obj){
			request_array.push(["SHOPADMINGOODSWHENEDIT", [{shop_goods_id:obj[i].id, shop_goods_when_sort:obj[i].value}]]);
		}
	...
	remove : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["SHOPADMINGOODSWHENREMOVE", [{shop_goods_id:ids[i]}]]);
		}
