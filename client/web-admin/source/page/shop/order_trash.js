WangAho({
	...
	main : function(){
		...
		WangAho("index").data({
			request : {
				list:["SHOPADMINORDERTRASHLIST", [config]]
			},
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/shop/order_trash.html", "#content"), data);
				_project.event();
	...
	restore : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["SHOPADMINORDERTRASHRESTORE", [{shop_order_id:ids[i]}]]);
		}
