WangAho({
	...
	main : function(){
		...
		if( (function(){try{ return _http.anchor.query.state;}catch(e){return false;}}()) ){
			config.search.merchant_state = _http.anchor.query.state;
		}
		...
		var request = {application_config:["APPLICATIONCONFIG"], list:["MERCHANTADMINLIST", [config]]};
		...
		WangAho("index").data({
			...
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/merchant/merchant_list.html", "#content"), data, {
		...
		$(".search").unbind("click").click(function(){
			...
			layer.open({
				title : "<span class=\"glyphicon glyphicon-filter\"></span> 筛选",
			  	...
			  	content: template( WangAho().template("page/merchant/merchant_list.html", "#search"), function(fn){
	...
	event : function(){
		...
		$('[action-button]').unbind("click").click(function(){
			...
			if("yes-self" == attr){
				layer.msg('你确定要设置为自营？('+ids.length+'条数据)', {time: 0 //不自动关闭
					,btn: ['确定', '取消']
					,yes: function(index){
					   layer.close(index);
					   _project.edit_yes_self(ids);
					}
				});
				return false;
			}
			...
			if("not-self" == attr){
				layer.msg('你确定要取消自营么？('+ids.length+'条数据)', {time: 0 //不自动关闭
					,btn: ['确定', '取消']
					,yes: function(index){
					   layer.close(index);
					   _project.edit_not_self(ids);
					}
				});
				return false;
			}
	...
	edit_yes_self : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["MERCHANTADMINEDITSELF", [{merchant_id:ids[i],merchant_self:1}]]);
		}
	...
	edit_not_self : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["MERCHANTADMINEDITSELF", [{merchant_id:ids[i],merchant_self:0}]]);
		}
	...
	remove : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["MERCHANTADMINREMOVE", [{merchant_id:ids[i]}]]);
		}
