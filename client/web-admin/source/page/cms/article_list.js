WangAho({
	...
	main : function(){
		...
		WangAho("index").data({
			request : {
				list:["CMSADMINARTICLELIST", [config]]
				}, 
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/cms/article_list.html", "#content"), data, {
	...
	search_event : function(){
		...
		$(".search").unbind("click").click(function(){
			...
			layer.open({
				title : "<span class=\"glyphicon glyphicon-filter\"></span> 筛选",
			  	...
			  	content: template( WangAho().template("page/cms/article_list.html", "#search"), function(fn){
		...
		$('[name="search-submit"]').unbind("click").click(function(){
			...
			var search = {};
			search.cms_article_id = $.trim($('[name="cms_article_id"]').val());
			search.cms_article_name = $.trim($('[name="cms_article_name"]').val());
	...
	event : function(){
		...
		$('[action-button]').unbind("click").click(function(){
			...
			if("state_publish" == attr){
				_project.state_publish(ids);
				return false;
			}
			
			//取消发布
			if("state_cancel_publish" == attr){
				_project.state_cancel_publish(ids);
				return false;
			}
	...
	sort : function(){
		...
		request_array.push(["CMSADMINARTICLEEDITCHECK"]);//第一个是判断是否有编辑权限
		for(var i in obj){
			request_array.push(["CMSADMINARTICLEEDIT", [{cms_article_id:obj[i].id, cms_article_sort:obj[i].value}]]);
		}
	...
	trash : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["CMSADMINARTICLETRASH", [{cms_article_id:ids[i]}]]);
		}
