WangAho({
	...
	main : function(){
		...
		WangAho("index").data({
			request : {
				list:["CMSADMINARTICLETRASHLIST", [config]]
			},
			success : function(data){
				...
				WangAho("index").view(WangAho().template("page/cms/article_trash.html", "#content"), data);
				_project.event();
	...
	restore : function(ids){
		...
		var request_array = [];
		for(var i in ids){
			request_array.push(["CMSADMINARTICLETRASHRESTORE", [{cms_article_id:ids[i]}]]);
		}
