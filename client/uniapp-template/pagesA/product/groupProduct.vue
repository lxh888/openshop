<template>
	<view class="container">
		...
		<view class="c-list" v-if="goodsInfo.goods_group" style="background-color: #DD1732;color:#FFF;">
			<view class="c-row b-b" >
				<!-- <text class="tit">团购商品</text> -->
				<view class="con" style="color:#FFF;display: flex;">
					<!-- <text class="selected-text" >
						{{goodsInfo.goods_group.start_time}}-{{goodsInfo.goods_group.end_time}}
					</text> -->
					<text>剩余</text>
					<bbsCountdown 
					:time="goodsInfo.goods_group.end_timestamp"
					:auto-start="true"
					:now="now"
				...
				<text class="yticon ">{{goodsInfo.goods_group.people}}人团 已有{{goodsInfo.goods_group.people_now}}人</text>
			</view>
		</view>
		...
		<view class="c-list">
			<view class="c-row b-b" @click="toAddress()">
		...
		<view class="page-bottom">
			...
			<view class="action-btn-group">
				<button type="primary" class=" action-btn no-border buy-now-btn" @click="orderGroup()">立即购买</button>
...
<script>
		...
		methods:{
			...
			orderGroup(){
				...
				if(!that.address_id){
					...
					ef.submit({
						request:{
							Overbooking:['SHOPORDERGROUPSELFFOUND',[{shop_goods_group_id:that.goodsGroupsID,user_address_id:that.address_id}]]
						},
						callback(data){
							var dataList=fns.checkError(data,'Overbooking',function(errno,error){
								uni.showToast({
									title:error,
									icon:'none'
								})
							})
							if(dataList.Overbooking && dataList.Overbooking.shop_order_group_id){
								uni.redirectTo({
									url:
										'../../pages/money/payGroup?order_id='+
										dataList.Overbooking.shop_order_group_id +
										'&money=' +
										that.goodsInfo.goods_group.price
										// +'&credit=' +credit
								})
							}
							
							console.log('打印',dataList)
						},
						error(err){
							fns.err('err',err,1)
						}
			...
			loadData(){
				...
				var addressID=uni.getStorageSync('nearByBusinessAddressID')
				
				var condition=''
				if(addressID){
					_this.address_id=addressID
					condition=[{id:_this.address_id}]
					uni.removeStorageSync('nearByBusinessAddressID')
					
				}
				else if(_this.address_id){
					condition=[{id:_this.address_id}]
				}
				...
				ef.submit({
					request: {
						...
						address:['USERADDRESSSELFGET',condition],
					...
					callback: function(data){
						...
						if(dataList.address && dataList.address!=''){
								...
								_this.userAddress=address.province+'-'+address.city+'-'+address.district+' '+address.details;
								_this.address_id=address.id
								console.log('address',address);
						}
						//团购id
						_this.goodsGroupsID=dataList.goods.goods_group.id;
						console.log('团',_this.goodsGroupsID)
						//快递类型
						// var arr =[]
						// if(dataList.express){
	...
	// 						   item.push(i)
			...
			toAddress(){
				var type = "3"//不知道这个type有什么值，取大一点
				uni.navigateTo({
					url: '../../pages/address/address?source=' + type
				})
			},
			toCommentList(){
				var _this=this;
