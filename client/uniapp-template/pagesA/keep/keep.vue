	...
	<view>
		...
		<view class="list">
			...
			<view class="sub-list goods" :class="subState">
				...
				<view class="row" v-for="(row,index) in goodsList" :key="index" >
					...
					<view class="carrier" :class="[typeClass=='goods'?theIndex==index?'open':oldIndex==index?'close':'':'']" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
						<view class="goods-info"  @tap="toGoods(row.shop_goods_id)">  <!-- -->
							<view class="img">
								<image :src="qiniu+row.shop_goods_image_main[0].image_id"></image>
							</view>
							<view class="info">
								<view class="title">{{row.shop_goods_image_main[0].shop_goods_name}}</view>
								<view class="price-number">
									<!-- <view class="keep-num">
										905人收藏
									</view> -->
									<view class="price">￥{{row.shop_goods_sku_min.shop_goods_sku_price/100}}</view>
			...
			<view class="sub-list shop" :class="subState">
				...
				<view class="row" v-for="(row,index) in shopList" :key="index"  ><!-- @tap="toshops(row.merchant_id)" -->
					<!-- 删除按钮 -->
					<view class="menu" >
					...
					<view class="carrier" :class="[typeClass=='shop'?theIndex==index?'open':oldIndex==index?'close':'':'']" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
						<view class="left">
							<image :src="qiniu+row.merchant_logo_image_id"></image>
						</view>
						<view class="right">
							<view class="sjname">
								{{row.merchant_name}}
							</view>
							<view  class="sjname"  style="font-size: 24upx; color: #7D7D7D;">
								{{row.merchant_info}}
							</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			load(){
				...
				ef.submit({
					request:{
						cel:['USERCOLLECTIONSELFLIST',[{"module":"shop_goods"}]],  //商品
						cel1:['USERCOLLECTIONSELFLIST',[{"module":"merchant"}]], //店铺
						numbber:['USERCOLLECTIONSELFCOUNT',[{"module":"shop_goods"}]],
						config: ['APPLICATIONCONFIG'], //配置信息
					},
					callback(data){
						var dataList = fns.checkError(data, ['cel','config','cel1','number'], function(errno, error) {
							fns.err(error);
						});
						console.log('datalist',dataList)
						if(dataList.cel){
							_this.goodsList=dataList.cel
							console.log('goodsList',_this.goodsList)
						}
						if(dataList.cel1){
							_this.shopList=dataList.cel1
						}
						//配置信息
			...
			// toshops(id){
			// 	uni.navigateTo({
			// 		url:'../../pagesA/BusinessDetails/BusinessDetails?id='+id
			// 	})
			// },
			//删除商品
