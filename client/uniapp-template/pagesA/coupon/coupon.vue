	...
	export default {
		...
		methods: {
			...
			getData(){
				...
				ef.submit({
					request:{
						g:['USERCOUPONSELFLIST',[{state:"1"}]], //1为未使用，2为已使用
						ov:['USERCOUPONSELFLIST',[{state:"2"}]]
					},
					callback:function(data){
						let dataList = fns.checkError(data,['g','ov'], function(errno, error) {
							uni.showToast({
								title: error,
								icon:'none'
							});
						...
						if(dataList.g){
							// console.log(dataList.g)
							let listCou = dataList.g.data.map((item,index)=>{
								return{
									id:index+1,
									title:item.name,
									termStart:item.start_times,
									termEnd:item.end_times,
									terStartime:item.start_time,
									terEndtime:item.end_time,
									ticket:item.discount/100,
									criteria:item.min==0?'无门槛':'满' + item.min/100 + '使用'
								}
							})
							console.log(listCou)
							that.couponValidList.push(...listCou)
							
						}
						if(dataList.ov){
							// console.log(dataList.g)
							let overlistCou = dataList.ov.data.map((item,index)=>{
								return{
									id:index+1,
									title:item.name,
									termStart:item.start_times,
							...
							// console.log(listCou)
							that.couponinvalidList.push(...overlistCou)
							// that.couponinvalidList = dataList.ov.data
						}
					}
				})
			},
