	...
	export default {
		...
		methods: {
					...
					ef.submit({
						...
						callback(data){
							...
							if(dataList.s){
								ef.submit({
									request: {
										s:[ 'SHOPORDERGROUPSELFPAYMENT',
										[{shop_order_group_id:_this.order_id,
					...
					wx.login({
						...
						success(res) {
							...
							if( res.code || (res.authResult && res.authResult.openid) ){
								...
								ef.submit({
									request: {
										s: ['SHOPORDERGROUPSELFPAYMENT', [shopOrderSelfAddConfig]]
									},
									callback(data){
										console.log('调起微信支付',data)
										/* uni.showToast({
			...
			paymoeny(password){
				...
				ef.submit({
					request:{
						s:['SHOPORDERGROUPSELFPAYMENT',[{shop_order_group_id:_this.order_id,pay_method:'user_money',pay_password:password}]]
					},
					callback(data) {
			...
			payjifen(password){
				...
				ef.submit({
					request:{
						s:['SHOPORDERGROUPSELFPAYMENT',[{shop_order_group_id:_this.order_id,pay_method:'user_credit',pay_password:password}]]
					},
					callback(data) {
					    const dataList=fns.checkError(data, 's', function(errno, error) {
						uni.showToast({
